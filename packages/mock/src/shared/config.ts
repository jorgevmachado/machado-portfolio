...
export function findAll<T = unknown>(req: Request, res: Response, mockEntity: MockEntity<T>) {
...
export function findOne<T = unknown>(req: Request, res: Response, mockEntity: MockEntity<T>) {
  const { param } = req.params;
  if(!param) {
    return buildResponse(res, notFoundException('Param is required.'));
  }
  const result = findOneEntity(param, mockEntity.list as Array<Record<string, unknown>>);
  if (!result) {
